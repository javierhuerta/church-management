...
La sección Calendario del sitio público se alimenta directamente del módulo Calendario
del admin: el sitio consume `GET /api/calendar` (que ya filtra a eventos `Published` para
visitantes anónimos vía `OptionalJwtAuthGuard`). No existe administración de eventos
separada para el sitio; la fuente de verdad es el módulo Calendario. Configuraciones
incluye una tab "Calendario" de tipo explicativa que documenta este flujo y enlaza al
módulo Calendario, y el sitio degrada con contenido por defecto si la API no responde.
