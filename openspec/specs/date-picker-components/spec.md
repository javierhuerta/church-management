...
Defines reusable date-selection UI components (`DatePicker`, `DateTimePicker`, `DateRangePicker`) built on the shadcn composition (`Popover` + `Calendar` / react-day-picker). They provide a consistent, Spanish-localized, timezone-safe, string-based API so all date fields in the frontend share one serialization boundary.
